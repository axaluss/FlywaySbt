...
object FlywayPlugin extends Plugin {
  ...
  lazy val flywaySettings :Seq[Setting[_]] = {
    ...
    Seq[Setting[_]](
      ...
      flywayMigrate <<= (externalDependencyClasspath in Compile, flywayConfig, streams) map {
        (cp, config, s) => withPrepared(cp, s) { Flyway(config).migrate() }
      },
      flywayValidate <<= (externalDependencyClasspath in Compile, flywayConfig, streams) map {
        (cp, config, s) => withPrepared(cp, s) { Flyway(config).validate() }
      },
      flywayInfo <<= (externalDependencyClasspath in Compile, flywayConfig, streams) map {
        (cp, config, s) => withPrepared(cp, s) { s.log.info(MigrationInfoDumper.dumpToAsciiTable(Flyway(config).info().all())) }
      },
      flywayRepair <<= (externalDependencyClasspath in Compile, flywayConfig, streams) map {
        (cp, config, s) => withPrepared(cp, s) { Flyway(config).repair() }
      },
      flywayClean <<= (externalDependencyClasspath in Compile, flywayConfig, streams) map {
        (cp, config, s) => withPrepared(cp, s) { Flyway(config).clean() }
      },
      flywayInit <<= (externalDependencyClasspath in Compile, flywayConfig, streams) map {
        (cp, config, s) => withPrepared(cp, s) { Flyway(config).init() }
      },
      flywayBaseline <<= (externalDependencyClasspath in Compile, flywayConfig, streams) map {
        (cp, config, s) => withPrepared(cp, s) { Flyway(config).baseline() }
      }
    )
  }
  private def withPrepared[T](  streams: TaskStreams)(f: => T): T = {
